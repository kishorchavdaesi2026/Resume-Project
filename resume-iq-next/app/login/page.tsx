...
export default function LoginPage() {
  ...
  const { user, login, loginOAuth, loginWithGoogleCredential, isLoading, error, clearError } = useAuthStore();
  ...
  const handleOAuth = async (provider: "google" | "linkedin") => {
    try {
      ...
      if (!isAppwriteConfigured) {
        addToast(`Welcome back! Logged in via ${provider === "google" ? "Google" : "LinkedIn"} sandbox credentials.`, "success");
        router.push("/dashboard");
      }
    } catch {
        ...
        <div className="w-full max-w-md mx-auto my-12 flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-sm text-slate-650 dark:text-slate-400">
              Sign in to ResumeIQ to manage and refine your career resumes.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 my-1">
            ...
            <span className="text-xs text-slate-400 dark:text-slate-500 uppercase tracking-wider">or email credentials</span>
            <hr className="border-slate-200 dark:border-white/5 flex-1" />
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <input
                  id="email"
                  type="email"
                  className={`w-full pl-10.5 pr-4 py-3 rounded-xl border bg-white/5 dark:bg-black/10 focus:bg-white dark:focus:bg-black/35 focus:outline-none focus:ring-2 focus:ring-brand-cyan/20 transition-all text-sm ${
                    errors.email
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex justify-between items-center">
                ...
                <Link
                  href="#"
                  onClick={() => addToast("Reset email utility requires active SMTP mail configuration.", "info")}
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="mt-2 w-full py-3.5 rounded-xl text-center text-sm font-bold text-white bg-gradient-to-r from-brand-cyan to-brand-blue shadow-lg hover:shadow-cyan-500/20 hover:scale-[1.01] transition-all flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50 disabled:pointer-events-none"
            >
      ...
      <section className="hidden lg:flex lg:w-[60%] bg-slate-900 relative items-center justify-center p-12">
        ...
        <div className="relative z-10 max-w-lg text-center flex flex-col gap-6 items-center">
          ...
          <h2 className="text-3xl font-bold font-display text-white tracking-tight">
            Design Resumes that Recruiter Systems Love.
          </h2>
